...
void get_rotated_rect(
    ...
    uint32_t cx,
    uint32_t cy,
    uint32_t sizex, // sizex is shape[1] or width
    uint32_t sizey, // sizey is shape[0] or height
    float angle) {

  cv::Mat src(height, width, CV_8UC(channels), (void*) input);
  Mat dst(sizey, sizex, CV_8UC(src.channels()), (void*) output);
  src.convertTo(src, CV_32FC(src.channels()), 1.0/255.0);

  cv::RotatedRect rr(cv::Point2f(cx, cy), Size(sizex, sizey), angle);

  // rotated rectangle
  Point2f rect_points[4];
  rr.points( rect_points );

  Mat temp(rr.size, CV_32FC(src.channels()));

  getRotRectImg(rr, src, temp);

  temp.convertTo(dst, CV_8UC(src.channels()), 255.0);
